...
from Pose2Sim.common import (
    convert_to_c3d,
    euclidean_distance,
    min_with_single_indices,
    reprojection,
    sort_stringlist_by_last_number,
...
from mocap.core.cameras import CameraSystem
...
def interpolate_zeros_nans(col, *args):
    ...
    if "kind" not in locals():  # 'linear', 'slinear', 'quadratic', 'cubic'
        f_interp = interpolate.interp1d(
            idx_good, col[idx_good], kind="linear", bounds_error=False
        )
    else:
        f_interp = interpolate.interp1d(
            idx_good,
    ...
    col_interp = np.where(
        mask, col, f_interp(col.index)
    )  # replace at false index with interpolated values

    # Reintroduce nans if length of sequence > N
    idx_notgood = np.where(~mask)[0]
    gaps = (
        np.where(np.diff(idx_notgood) > 1)[0] + 1
    )  # where the indices of true are not contiguous
    sequences = np.split(idx_notgood, gaps)
    if sequences[0].size > 0:
        for seq in sequences:
            if (
                len(seq) > N
            ):  # values to exclude from interpolation are set to false when they are too long
...
def sort_people(Q_kpt_old, Q_kpt):
    ...
    if len(Q_kpt_old) < len(Q_kpt):
        Q_kpt_old = np.concatenate(
            (Q_kpt_old, [[0.0, 0.0, 0.0, 1.0]] * (len(Q_kpt) - len(Q_kpt_old)))
        )
    if len(Q_kpt) < len(Q_kpt_old):
        Q_kpt = np.concatenate(
            (Q_kpt, [[0.0, 0.0, 0.0, 1.0]] * (len(Q_kpt_old) - len(Q_kpt)))
        )
    personsIDs_comb = sorted(it.product(range(len(Q_kpt_old)), range(len(Q_kpt))))
    ...
    minL, _, associated_tuples = min_with_single_indices(
...
def retrieve_right_trc_order(trc_paths):
    ...
    while retry:
        ...
        [
            logging.info(f"#{t_list}: {os.path.basename(trc_list)}")
            for t_list, trc_list in enumerate(trc_paths)
        ]
        trc_id = []
...
def recap_triangulate(
    ...
    # if single trial
    session_dir = (
        session_dir if "Config.toml" in os.listdir(session_dir) else os.getcwd()
    )
    calib_dir = [
    ...
    calib_file = glob.glob(os.path.join(calib_dir, "*.toml"))[
        0
    ]  # lastly created calibration file
    ...
    cam_names = np.array(
        [calib[c].get("name") if calib[c].get("name") else c for c in cal_keys]
    )
    cam_names = cam_names[list(cam_excluded_count[0])]
    error_threshold_triangulation = config_dict.get("triangulation").get(
        "reproj_error_threshold_triangulation"
    )
    likelihood_threshold = config_dict.get("triangulation").get(
    ...
    interp_gap_smaller_than = config_dict.get("triangulation").get(
    ...
    for n in range(nb_persons_to_detect):
        ...
        for idx, name in enumerate(keypoints_names):
            mean_error_keypoint_px = np.around(
                error[n].iloc[:, idx].mean(), decimals=1
            )  # RMS à la place?
            mean_error_keypoint_m = np.around(
                mean_error_keypoint_px * Dm / fm, decimals=3
            )
            mean_cam_excluded_keypoint = np.around(
                nb_cams_excluded[n].iloc[:, idx].mean(), decimals=2
            )
            logging.info(
            ...
            if show_interp_indices:
                if interpolation_kind != "none":
                    if (
                        len(list(interp_frames[n][idx])) == 0
                        and len(list(non_interp_frames[n][idx])) == 0
                    ):
                        logging.info("  No frames needed to be interpolated.")
                    if len(list(interp_frames[n][idx])) > 0:
                        interp_str = (
                            str(interp_frames[n][idx])
                            .replace(":", " to ")
                            .replace("'", "")
                            .replace("]", "")
                            .replace("[", "")
        ...
        logging.info(
            f"In average, {mean_cam_excluded} cameras had to be excluded to reach these thresholds."
        )
        cam_excluded_count[n] = dict(zip(cam_names, cam_excluded_count[n].values()))
        cam_excluded_count[n] = dict(
            sorted(
                cam_excluded_count[n].items(), key=lambda item: item[1], reverse=True
            )
        )
        ...
        for i, (k, v) in enumerate(cam_excluded_count[n].items()):
            if i == 0:
                str_cam_excluded_count += (
                    f"Camera {k} was excluded {int(np.round(v*100))}% of the time, "
                )
    ...
    logging.info(
        f'Lens distortions were {"taken into account" if undistort_points else "not taken into account"}.'
    )
...
def triangulation_from_best_cameras(
    ...
    min_cameras_for_triangulation = config_dict.get("triangulation").get(
    ...
    nb_cams_off = (
        0  # cameras will be taken-off until reprojection error is under threshold
    )
    # print('\n')
    while (
        error_min > error_threshold_triangulation
        and n_cams - nb_cams_off >= min_cameras_for_triangulation
    ):
        ...
        id_cams_off_tot_new = [
            np.argwhere(np.isnan(x)).ravel() for x in likelihood_files_filt
        ]
        ...
        if undistort_points:
            calib_params_K_filt = [
                [
                    ...
                    if not np.isnan(likelihood_files_filt[j][i])
                    and likelihood_files_filt[j][i] != 0.0
                ]
        ...
        x_files_filt = [
            np.array(
                [
                    ...
                    if not np.isnan(likelihood_files_filt[i][ii])
                    and likelihood_files_filt[i][ii] != 0.0
                ]
            )
        ...
        likelihood_files_filt = [
            np.array([xx for ii, xx in enumerate(x) if not np.isnan(xx) and xx != 0.0])
        ...
        Q_filt = [
            weighted_triangulation(
                projection_matrices_filt[i],
                x_files_filt[i],
                y_files_filt[i],
                likelihood_files_filt[i],
            )
            for i in range(len(id_cams_off))
        ]
        ...
        if undistort_points:
            ...
            coords_2D_kpt_calc_filt = [
                [coords_2D_kpt_calc_filt[i][:, 0], coords_2D_kpt_calc_filt[i][:, 1]]
                for i in range(len(id_cams_off))
            ]
        else:
            coords_2D_kpt_calc_filt = [
                reprojection(projection_matrices_filt[i], Q_filt[i])
                for i in range(len(id_cams_off))
            ]
        ...
        for config_off_id in range(len(x_calc_filt)):
            ...
            error.append(
                np.mean(
                    [
                        euclidean_distance(q_file[i], q_calc[i])
                        for i in range(len(q_file))
                    ]
                )
            )
        # print('error ', error)
        ...
        if handle_LR_swap and error_min > error_threshold_triangulation:
            ...
            while (
                error_off_swap_min > error_threshold_triangulation
                and n_cams_swapped < (n_cams - nb_cams_off_tot) / 2
                ...
                # Create subsets
                id_cams_swapped = np.array(
                    list(
                        it.combinations(range(n_cams - nb_cams_off_tot), n_cams_swapped)
                    )
                )
                # print('id_cams_swapped ', id_cams_swapped)
                x_files_filt_off_swap = [
                    [x] * len(id_cams_swapped) for x in x_files_filt
                ]
                y_files_filt_off_swap = [
                    [y] * len(id_cams_swapped) for y in y_files_filt
                ]
                # print('x_files_filt_off_swap ', x_files_filt_off_swap)
                # print('y_files_filt_off_swap ', y_files_filt_off_swap)
                for id_off in range(
                    len(id_cams_off)
                ):  # for each configuration with nb_cams_off_tot removed
                    for id_swapped, config_swapped in enumerate(
                        ...
                        x_files_filt_off_swap[id_off][id_swapped][config_swapped] = (
                            x_files_swapped_filt[id_off][config_swapped]
                        )
                        y_files_filt_off_swap[id_off][id_swapped][config_swapped] = (
                            y_files_swapped_filt[id_off][config_swapped]
                        )
                ...
                if undistort_points:
                    ...
                    coords_2D_kpt_calc_off_swap = np.array(
                        [
                            [
                                [
                                    coords_2D_kpt_calc_off_swap[id_off][
                                        id_swapped, :, 0
                                    ],
                                    coords_2D_kpt_calc_off_swap[id_off][
                                        id_swapped, :, 1
                    ...
                    coords_2D_kpt_calc_off_swap = [
                        np.array(
                            [
                                reprojection(
                                    projection_matrices_filt[id_off],
                                    Q_filt_off_swap[id_off][id_swapped],
                                )
                                for id_swapped in range(len(id_cams_swapped))
                            ]
                        )
                ...
                for id_off in range(len(id_cams_off)):
                    ...
                    for id_swapped, config_swapped in enumerate(id_cams_swapped):
                        ...
                        q_file_off_swap = [
                            (
                                x_files_filt_off_swap[id_off][id_swapped][i],
                                y_files_filt_off_swap[id_off][id_swapped][i],
                            )
                            for i in range(n_cams - nb_cams_off_tot)
                        ]
                        q_calc_off_swap = [
                            (
                                x_calc_off_swap[id_off][id_swapped][i],
                                y_calc_off_swap[id_off][id_swapped][i],
                            )
                            for i in range(n_cams - nb_cams_off_tot)
                        ]
                        error_percam.append(
                            np.mean(
                                [
                                    euclidean_distance(
                                        q_file_off_swap[i], q_calc_off_swap[i]
                                    )
                ...
                best_off_swap_config = np.unravel_index(
                    error_off_swap.argmin(), error_off_swap.shape
                )
            ...
            try:
                with open(json_tracked_files_f[cam_nb], "r") as json_f:
                    ...
                    for keypoint_id in keypoints_ids:
                        try:
                            x_files_cam.append(
                                js["people"][n]["pose_keypoints_2d"][keypoint_id * 3]
                            )
                            y_files_cam.append(
                                js["people"][n]["pose_keypoints_2d"][
                                    keypoint_id * 3 + 1
                                ]
                            )
                            likelihood_files_cam.append(
                                js["people"][n]["pose_keypoints_2d"][
                                    keypoint_id * 3 + 2
                                ]
                            )
...
def triangulate_all(experiment):
    """
    For each frame
    For each keypoint
    ...
    - a .trc file with 3D coordinates in Y-up system coordinates
    """
    project_dir = experiment.path
    calib_file = experiment.calibration_file
    config_dict = experiment.cfg
    config_dict["project"]["project_dir"] = project_dir
    ...
    calib_params = CameraSystem(calib_file).to_dict(undistort=undistort_points)
    P = calib_params["P"]
    ...
    keypoints_names_swapped = [
        keypoint_name.replace("R", "L")
        if keypoint_name.startswith("R")
        else keypoint_name.replace("L", "R")
        if keypoint_name.startswith("L")
        else keypoint_name
        for keypoint_name in keypoints_names
    ]
    keypoints_names_swapped = [
        keypoint_name_swapped.replace("right", "left")
        if keypoint_name_swapped.startswith("right")
        else keypoint_name_swapped.replace("left", "right")
        if keypoint_name_swapped.startswith("left")
        else keypoint_name_swapped
        for keypoint_name_swapped in keypoints_names_swapped
    ]
    keypoints_idx_swapped = [
        keypoints_names.index(keypoint_name_swapped)
        for keypoint_name_swapped in keypoints_names_swapped
    ]  # find index of new keypoint_name
    ...
    try:
        json_files_names = [
            fnmatch.filter(os.listdir(os.path.join(poseTracked_dir, js_dir)), "*.json")
        ...
        try:
            json_files_names = [
                fnmatch.filter(os.listdir(os.path.join(poseSync_dir, js_dir)), "*.json")
                for js_dir in json_dirs_names
            ]
            ...
            try:
                json_files_names = [
                    fnmatch.filter(os.listdir(os.path.join(pose_dir, js_dir)), "*.json")
                    for js_dir in json_dirs_names
                ]
            except:
                raise Exception(
    ...
    f_range = [
        [0, min([len(j) for j in json_files_names])]
        if frame_range == []
        else frame_range
    ][0]
    ...
    # Triangulation
    if multi_person:
        nb_persons_to_detect = max(
            max(
                count_persons_in_json(
                    os.path.join(pose_dir, json_dirs_names[c], json_fname)
                )
                for json_fname in json_files_names[c]
            )
    ...
    for f in tqdm(range(*f_range)):
        ...
        json_files_names_f = [
            [j for j in json_files_names[c] if int(re.split(r"(\d+)", j)[-2]) == f]
            for c in range(n_cams)
        ]
        json_files_names_f = [
            j for j_list in json_files_names_f for j in (j_list or ["none"])
        ]
        json_files_f = [
            os.path.join(pose_dir, json_dirs_names[c], json_files_names_f[c])
            for c in range(n_cams)
        ]

        x_files, y_files, likelihood_files = extract_files_frame_f(
            json_files_f, keypoints_ids, nb_persons_to_detect
        )
        ...
        if undistort_points:
            for n in range(nb_persons_to_detect):
                points = [
                    np.array(tuple(zip(x_files[n][i], y_files[n][i])))
                    .reshape(-1, 1, 2)
                    .astype("float32")
                    for i in range(n_cams)
                ]
                undistorted_points = [
                    cv2.undistortPoints(
                        ...
                        calib_params["dist"][i],
                        None,
                        calib_params["optim_K"][i],
                    )
                    for i in range(n_cams)
                ]
                x_files[n] = np.array(
                    [[u[i][0][0] for i in range(len(u))] for u in undistorted_points]
                )
                y_files[n] = np.array(
                    [[u[i][0][1] for i in range(len(u))] for u in undistorted_points]
                )
        ...
        for n in range(nb_persons_to_detect):
            for keypoint_idx in keypoints_idx:
                ...
                coords_2D_kpt = np.array(
                    (
                        x_files[n][:, keypoint_idx],
                        y_files[n][:, keypoint_idx],
                        likelihood_files[n][:, keypoint_idx],
                    )
                )
                ...
                Q_kpt, error_kpt, nb_cams_excluded_kpt, id_excluded_cams_kpt = (
        ...
        if multi_person and f != 0:
            ...
            for i in range(len(Q)):
                id_in_old = associated_tuples[:, 1][
                    associated_tuples[:, 0] == i
                ].tolist()
                if len(id_in_old) > 0:
                    personsIDs_sorted += id_in_old
            ...
            error, nb_cams_excluded, id_excluded_cams = (
                error_sorted,
                nb_cams_excluded_sorted,
                id_excluded_cams_sorted,
            )
        ...
        nb_cams_excluded_tot.append(
            [nb_cams_excluded[n] for n in range(nb_persons_to_detect)]
        )
        id_excluded_cams = [
            [id_excluded_cams[n][k] for k in range(keypoints_nb)]
            for n in range(nb_persons_to_detect)
        ]
    ...
    Q_tot = [
        list(tpl)
        for tpl in zip(*it.zip_longest(*Q_tot, fillvalue=[np.nan] * keypoints_nb * 3))
    ]
    error_tot = [
        list(tpl)
        for tpl in zip(
            *it.zip_longest(*error_tot, fillvalue=[np.nan] * keypoints_nb * 3)
        )
    ]
    nb_cams_excluded_tot = [
        list(tpl)
        for tpl in zip(
            *it.zip_longest(
                *nb_cams_excluded_tot, fillvalue=[np.nan] * keypoints_nb * 3
            )
        )
    ]
    id_excluded_cams_tot = [
        list(tpl)
        for tpl in zip(
            *it.zip_longest(
                *id_excluded_cams_tot, fillvalue=[np.nan] * keypoints_nb * 3
            )
        )
    ]
    ...
    Q_tot = [
        pd.DataFrame([Q_tot_f[n] for Q_tot_f in Q_tot])
        for n in range(nb_persons_to_detect)
    ]
    error_tot = [
        pd.DataFrame([error_tot_f[n] for error_tot_f in error_tot])
        for n in range(nb_persons_to_detect)
    ]
    nb_cams_excluded_tot = [
        pd.DataFrame(
            [
                nb_cams_excluded_tot_f[n]
                for nb_cams_excluded_tot_f in nb_cams_excluded_tot
            ]
        )
        for n in range(nb_persons_to_detect)
    ]
    id_excluded_cams_tot = [
        pd.DataFrame(
            [
                id_excluded_cams_tot_f[n]
                for id_excluded_cams_tot_f in id_excluded_cams_tot
            ]
        )
        for n in range(nb_persons_to_detect)
    ]
    ...
    zero_nan_frames = [
        np.where(Q_tot[n].iloc[:, ::3].T.eq(0) | ~np.isfinite(Q_tot[n].iloc[:, ::3].T))
        for n in range(nb_persons_to_detect)
    ]
    zero_nan_frames_per_kpt = [
        [
            zero_nan_frames[n][1][np.where(zero_nan_frames[n][0] == k)[0]]
            for k in range(keypoints_nb)
        ]
        for n in range(nb_persons_to_detect)
    ]
    non_nan_nb_first_kpt = [
        frame_nb - len(zero_nan_frames_per_kpt[n][0])
        for n in range(nb_persons_to_detect)
    ]
    deleted_person_id = [
        n for n in range(len(non_nan_nb_first_kpt)) if non_nan_nb_first_kpt[n] < 4
    ]

    Q_tot = [Q_tot[n] for n in range(len(Q_tot)) if n not in deleted_person_id]
    error_tot = [
        error_tot[n] for n in range(len(error_tot)) if n not in deleted_person_id
    ]
    nb_cams_excluded_tot = [
        nb_cams_excluded_tot[n]
        for n in range(len(nb_cams_excluded_tot))
        if n not in deleted_person_id
    ]
    id_excluded_cams_tot = [
        id_excluded_cams_tot[n]
        for n in range(len(id_excluded_cams_tot))
        if n not in deleted_person_id
    ]
    ...
    id_excluded_cams_tot = [
        np.hstack(np.hstack(np.array(id_excluded_cams_tot[n])))
        for n in range(nb_persons_to_detect)
    ]
    cam_excluded_count = [dict(Counter(k)) for k in id_excluded_cams_tot]
    [
        cam_excluded_count[n].update(
            (x, y / frame_nb / keypoints_nb) for x, y in cam_excluded_count[n].items()
        )
        for n in range(nb_persons_to_detect)
    ]
    ...
    if show_interp_indices:
        gaps = [
            [
                np.where(np.diff(zero_nan_frames_per_kpt[n][k]) > 1)[0] + 1
                for k in range(keypoints_nb)
            ]
            for n in range(nb_persons_to_detect)
        ]
        sequences = [
            [
                np.split(zero_nan_frames_per_kpt[n][k], gaps[n][k])
                for k in range(keypoints_nb)
            ]
            for n in range(nb_persons_to_detect)
        ]
        interp_frames = [
            [
                [
                    f"{seq[0]}:{seq[-1]}"
                    for seq in seq_kpt
                    if len(seq) <= interp_gap_smaller_than and len(seq) > 0
                ]
                for seq_kpt in sequences[n]
            ]
            for n in range(nb_persons_to_detect)
        ]
        non_interp_frames = [
            [
                [
                    f"{seq[0]}:{seq[-1]}"
                    for seq in seq_kpt
                    if len(seq) > interp_gap_smaller_than
                ]
                for seq_kpt in sequences[n]
            ]
            for n in range(nb_persons_to_detect)
    ...
    # Create TRC file
    trc_paths = [
        make_trc(config_dict, Q_tot[n], keypoints_names, f_range, id_person=n)
        for n in range(len(Q_tot))
    ]
    if make_c3d:
