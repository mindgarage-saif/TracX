...
def estimation_2d_custom(
    ...
    pose_model_path = os.path.join(APP_ASSETS, pose_model_path)
    pose_model = RTMPose(
    ...
    for video_file in video_files:
        video_name_without_ext = (
            os.path.splitext(os.path.basename(video_file))[0] + "_json"
        )
        ...
        with tqdm(
            total=total_frames,
            desc=f"Processing {video_name_without_ext}",
        ) as pbar:
            while cap.isOpened():
                ret, frame = cap.read()
                if not ret:
                ...
                json_file_path = os.path.join(
                    json_output_dir,
                    video_name_without_ext,
                    f"{video_name_without_ext}_{frame_idx:06d}.json",
                )
