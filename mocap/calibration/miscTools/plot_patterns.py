...
def plot_chessboard(c_pattern, p_width, p_height, w, h):
    p_width += 1
    p_height += 1
    w_step, w_down, w_limit, h_step, h_down, h_limit = pre_plot(p_width, p_height, w, h)
    ...
    for i in range(w_down, w_limit + 1, w_step):
        ...
        if i == w_down:
            c_pattern.create_oval(
                [
                    (i + w_step - w_step / 5.0, h_down + h_step - h_step / 5.0),
                    (i + w_step + w_step / 5.0, h_down + h_step + h_step / 5.0),
                ],
                fill=color_pattern_mix,
                tag="grid_oval",
            )
    ...
    for i in range(h_down, h_limit + 1, h_step):
        ...
        if h_down < i < h_limit - 1 - h_step:
            c_pattern.create_oval(
                [
                    (w_down + w_step - w_step / 5.0, i + h_step - h_step / 5.0),
                    (w_down + w_step + w_step / 5.0, i + h_step + h_step / 5.0),
                ],
                tag="grid_oval",
                fill=color_pattern_height,
            )
        ...
        if symmetric is True:
            ...
            for j in range(int(i % 2 != 0), p_height, 2):
                ...
                if j == int(i % 2 != 0):
                    if i == 0:
                        c_pattern.create_oval(
                            [(index_i, index_j), (index_i + w_step, index_j + h_step)],
                            fill=color_pattern_mix,
                            tag="grid_oval",
                        )
                    else:
...
def plot_custom(c_pattern, all_points, w, h):
    ...
    points = [
        (randGenerator.randint(0, w - 1), randGenerator.randint(0, h - 1))
