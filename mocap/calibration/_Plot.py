...
class Mixin:
    ...
    def click_to_zoom(self, event, camera):
        ...
        # maximum
        if (
            self.btn_zoom_more.config("relief")[-1] == "sunken"
        ...
        # minimum
            ...
            self.btn_zoom_less.config("relief")[-1] == "sunken" and self.zoomhandler > 0
        ):
            self.scale *= self.delta
            self.imscale *= self.delta
            self.zoomhandler -= 1
        # check if there are loaded images and if the locate button in enable
            ...
            and self.tabControl[camera].index(self.tabControl[camera].select()) == 1
        ):
    ...
    def change_position_feature(self):
        ...
        for camera in range(self.n_cameras):
            if self.new_coord_feature[camera]:
                self.detected_features[camera][self.index.get()][
                    self.index_corner.get()
                ] = self.new_coord_feature[camera]
        self.popup.destroy()
        self.updatePicture()
    ...
    def updatePicture(self, *args):
        ...
        if selection >= 0:
            ...
            for j in range(self.n_cameras):
                ...
                images[j].append(Image.fromarray(self.img_original[j][selection]))
                ...
                images[j].append(Image.fromarray(self.heat_map[j]))
        ...
        for j in range(self.n_cameras):
            ...
            for i in range(len(self.paths[0])):
                ...
                points = [
                    0,
                    self.p_height - 1,
                    self.p_width * self.p_height - 1,
                    self.p_width * self.p_height - self.p_height,
                ]
                for p in points:
    ...
    def show_moving_features(self, camera, index):
        ...
        for index_f in range(len(self.detected_features[camera][index])):
            ...
            if index_f == self.index_corner.get():
                ...
                if self.new_coord_feature[camera]:
                    if (
                        abs(a[0][0] - self.new_coord_feature[camera][0][0]) < 10**-3
                        and abs(a[0][1] - self.new_coord_feature[camera][0][1]) < 10**-3
                    ):
                        color = (80, 149, 200)
    ...
    def project_detected_features(self, camera, index, forExtrinsics=False):
        ...
        for i in range(self.p_height):
            for j in range(self.p_width):
                ...
                if i < self.p_height - 1:
                    b = self.detected_features[camera][index][
                        j * self.p_height + i + 1
                    ].astype(np.uint8)
                    cv2.line(
                        im3,
                ...
                if data is not None:
                    ...
                    if self.ax[k][j].patches:
                        ...
                        self.ax[k][j].set_yticklabels(
                            [
                                item.get_text()
                                for item in self.ax[k][j].get_yticklabels()
                            ],
                            size=10,
                        )
                    ...
                    if k == 0:
                        ...
                        self.ax[k][j].legend(
                            handles[0:1],
                            [self._("Mean RMS Reprojection Error is: %.5f") % m_error],
                            loc="lower center",
                    ...
                    if self.update:
                        ...
                        self.ax[k][j].set_title(
                            title_names[k] + self._(" (Not Updated)"),
                            color="#9a1046",
                            fontsize=10,
                        )
