...
def BodySkeletionLayer(pose,transl,Projection_mat,img_size,J,j_to_idx):
 ...
 '''
 order = [j_to_idx['Hip'],j_to_idx['RHip'],j_to_idx['RKnee'],j_to_idx['RAnkle'],j_to_idx['RBigToe'],j_to_idx['RSmallToe'],j_to_idx['RHeel'],j_to_idx['LHip'],j_to_idx['LKnee'],j_to_idx['LAnkle'],j_to_idx['LBigToe'],j_to_idx['LSmallToe'],j_to_idx['LHeel'],#12
          j_to_idx['Neck'],j_to_idx['Head'],j_to_idx['Nose'],j_to_idx['RShoulder'],j_to_idx['RElbow'],j_to_idx['RWrist'],j_to_idx['LShoulder'],j_to_idx['LElbow'],j_to_idx['LWrist']]
 J = J[order] 
 parents =  np.array([-1, 0, 1, 2, 3, 3, 3, 0, 7, 8, 9, 9, 9, 0,13,14,13,16,17,13,19,20])
...
def get_2D_joints(path_to_2d_joints,which_person=0):
    joints_2d = []
    confidences=[]
    for file in sorted(os.listdir(path_to_2d_joints), key=lambda x: int(x.split('_')[1])):
    ...
    return np.array(joints_2d,dtype=np.float64), np.array(confidences,dtype=np.float64)

def main_HPE(path_to_2d_joints1,path_to_2d_joints2,path_to_2d_joints3,path_to_3d_joints):
    joint_name_to_idx = {
        "Nose": 0,
        "LEye": 1,
        "REye": 2,
    ...
    joint_gt_2d_views = np.array([joints_gt_2D_all_view1,joints_gt_2D_all_view2,joints_gt_2D_all_view3])
    joints_3d, joint_names_to_idx = readTrc(path_to_3d_joints)
    ordering = [joint_name_to_idx['Hip'],joint_name_to_idx['RHip'],joint_name_to_idx['RKnee'],joint_name_to_idx['RAnkle'],joint_name_to_idx['RBigToe'],joint_name_to_idx['RSmallToe'],joint_name_to_idx['RHeel'],joint_name_to_idx['LHip'],joint_name_to_idx['LKnee'],joint_name_to_idx['LAnkle'],joint_name_to_idx['LBigToe'],joint_name_to_idx['LSmallToe'],joint_name_to_idx['LHeel'],joint_name_to_idx['Neck'],joint_name_to_idx['Head'],joint_name_to_idx['Nose'],joint_name_to_idx['RShoulder'],joint_name_to_idx['RElbow'],joint_name_to_idx['RWrist'],joint_name_to_idx['LShoulder'],joint_name_to_idx['LElbow'],joint_name_to_idx['LWrist']]
    enhanced_3d_joints = []
    reprojection_error = []
    for j in range(len(joints_gt_2D_all_view1)):
        ...
        P = np.array([[P_new[0][0],P_new[0][1],P_new[0][2]],[P_new[1][0],P_new[1][1],P_new[1][2]],[P_new[2][0],P_new[2][1],P_new[2][2]]])
        ...
        joint_gt = torch.from_numpy(np.array([joint_gt_2D_view1,joint_gt_2D_view2,joint_gt_2D_view3]))
        #joint_gt = torch.from_numpy(np.array([joint_gt_2D_view3]))
        joint_gt = torch.unsqueeze(joint_gt, dim=0).view(1,3,joint_num,2)
        #print(P)
        confidences_tensor = torch.from_numpy(np.array([confidences_view1,confidences_view2,confidences_view3]))
        #confidences_tensor = torch.from_numpy(np.array([confidences_view3]))
        confidences_tensor = torch.unsqueeze(confidences_tensor, dim=0).view(1,3,joint_num,1)
        ...
        h_keys_proj_gt = normalize_keys_batch(joint_gt.float(), img_size[1], img_size[0])
        #h_keys_proj_gt = multiview_projection_batch(Ps_batch.detach(), joint_gt.float())
        ...
        vari_dict["transl"] = init_transl.view(1,3).clone().detach().requires_grad_(True)
        vari_dict["pose"] = torch.zeros((joint_num*3), requires_grad=True)
        params = [{"params": vari_dict["pose"], "lr": 0.05},
        ...
        for i in range(itr):
            joints_pos,h_keys_proj = BodySkeletionLayer(vari_dict["pose"], vari_dict["transl"],Ps_batch,img_size,joints_3d[j],joint_names_to_idx)
            #loss1 = (torch.mul((h_keys_proj-h_keys_proj_gt).pow(2),confidences_tensor))
            #loss1[:,:,indices,:] *= 10
            loss = (torch.mul((h_keys_proj - h_keys_proj_gt).pow(2), confidences_tensor) * torch.tensor([mult_dict[i] if i in indices else 1 for i in range(h_keys_proj.size(2))], dtype=h_keys_proj.dtype, device=h_keys_proj.device).view(1, 1, -1, 1)).mean()\
                #+0.000006*vari_dict['pose'].pow(2).mean()  # test with 2d loss
    ...
    for k in range(3):
        for i in range(joint_num):
            ...
            joint_gt = torch.from_numpy(np.array([joint_gt_2d_views[k,2460][ordering]]))
            ...
            joint_gt2 = torch.from_numpy(np.array([joint_gt_2d_views[k,2620][ordering]]))
            joint_gt2 = torch.unsqueeze(joint_gt2, dim=0).view(1,1,joint_num,2)
            joint_gt2 = normalize_keys_batch(joint_gt2.float(), img_size[1], img_size[0])
...
if __name__ == '__main__':
    path_to_2d_joints1 = os.path.expanduser(r"E:\Uni\Data\LIHS\2dFitting\RTMPOSE_L\ROM2\23087_json")
    path_to_2d_joints2 = os.path.expanduser(r"E:\Uni\Data\LIHS\2dFitting\RTMPOSE_L\ROM2\29092_json")
    path_to_2d_joints3 = os.path.expanduser(r"E:\Uni\Data\LIHS\2dFitting\RTMPOSE_L\ROM2\29913_json")
    path_to_3d_joints = os.path.expanduser(r"C:\Users\Jeremias\anaconda3\envs\Pose2Sim\Lib\site-packages\Pose2Sim\RMTPoseL\RMTPoseL\ROM2\3c\23087_29092_29913\ROM2_RTMPose_0-3038_filt_butterworth.trc")
    #joints_3d, joint_names_to_idx = readTrc()
    main_HPE(path_to_2d_joints1,path_to_2d_joints2,path_to_2d_joints3,path_to_3d_joints)
