...
def instrinsic2string(c, d):
    ...
    if c[0][0] == 0:
        A = "Camera Matrix \n" + "\n".join(
            "\t".join("???.???" for x in y) for y in np.eye(3)
        )
        B = "Distortion Coefficients \n" + "\n".join(
            "\t".join("???.???" for x in y) for y in np.zeros((5, 1))
        )
    else:
        A = "Camera Matrix \n" + "\n".join(
            "\t".join(s_decimals % x for x in y) for y in c
        )
        B = "Distortion Coefficients \n" + "\n".join(
            "\t".join(s_decimals % x for x in y) for y in d
        )
    return A + "\n\n" + B
...
def extrinsic2string(R, T):
    ...
    if R is None:
        ...
        A = "Rotation Matrix between Cameras \n" + "\n".join(
            "\t".join(s_decimals % x for x in y) for y in R
        )
        B = "Translational Vector between Cameras \n" + "\n".join(
            "\t".join(s_decimals % x for x in y) for y in T
        )
    return A + "\n\n" + B
