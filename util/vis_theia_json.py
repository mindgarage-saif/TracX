...
def vis_theia_json(json_path):
    ...
    for frame_index, frame in enumerate(frames):
        ...
        for i, point in enumerate(segmentPos):
            points = ax.scatter(
                point[0], point[1], point[2], label=f"{i} {lableMarkers[i]}"
            )
            ax.text(
                point[0],
                point[1],
            ...
            if i in connections:
                for connection in connections[i]:
                    ...
                    o += 1
                    (line,) = ax.plot(
                        [point[0], segmentPos[connection][0]],
                        [point[1], segmentPos[connection][1]],
                        [point[2], segmentPos[connection][2]],
                        color=color,
                    )
...
if __name__ == "__main__":
    vis_theia_json(
        r"E:\Uni\Data\LIHS\LIHS\ROM_2\ROM_2\Generic Markerless 2-3d-data.json"
    )
