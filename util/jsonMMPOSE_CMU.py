            ...
            for frame in data:
                ...
                for instance in frame['instances']:
                    ...
                    if rotation_angle == '270':
                        ...
                        x2 = width - np.array(points[:,1]) # static 1920 for width of image
                    elif rotation_angle == '90':
                        x2 = np.array(points[:,1])
                        y2 = height - np.array(points[:,0]) # static 1088 for height of image
                    ...
                    pose_keypoints_2d = [coord for i in range(len(rotated_points)) for coord in [rotated_points[i][0], rotated_points[i][1], c[i]]]
                ...
                new_filename = f"{filename.split('.')[0]}_{frame['frame_id']:012d}_keypoints.json"
...
if __name__ == '__main__':
    ...
    argsparser.add_argument("--json_2d_path_mmpose",default=r"~/Documents/Pose2SimData/RTMPose-t/ROM_2_videos_rot", type=str, help="Path to the directory containing the JSON files")
    argsparser.add_argument("--xml_path",default=r"~/Documents/Pose2SimData/RTMPose-t/Gait Markerless 1.settings.xml", type=str, help="Path to the camera XML file")
    argsparser.add_argument("--output_path",default=r"~/Documents/Pose2SimData/RTMPose-t/ROM_2_videos_rot", type=str, help="Path to the output directory")
