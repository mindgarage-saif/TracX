...
def blur(path, region, blur_size, output):
    ...
    out = cv2.VideoWriter(output, cv2.VideoWriter_fourcc('M','J','P','G'), fps, (frame_width, frame_height))
...
if __name__ == "__main__":
    ...
    argsPars.add_argument("--path",default=r"./video.avi", type=str, help="Path to the video file")
    argsPars.add_argument("--region", nargs=4, type=int, help="Region to blur (top left corner to bottom right corner)")
    argsPars.add_argument("--blur_size",nargs=2, type=int, help="Size of the blur filter")
    argsPars.add_argument("--output",default=r"./video_blur.avi", type=str, help="Path to the output video file")
