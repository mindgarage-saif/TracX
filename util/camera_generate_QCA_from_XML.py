...
def parseXML(in_path,out_path,camera_list,path_to_project,switch):
    ...
    with open(os.path.join(path_to_project,file_name.replace(".xml",".qca.txt")), 'w') as txt_file:
        txt_file.write("<?xml version='1.0' encoding='ASCII'?>\n")
        txt_file.write(xml_content)

if __name__ == '__main__':
    argspars = argparse.ArgumentParser()
    argspars.add_argument("--in_path",default=r'./Gait Markerless 2.settings.xml', type=str, help="Path to the input XML file")
    argspars.add_argument("--out_path",default=r'./Gait Markerless 2.settings_new.xml', type=str, help="Path to the output XML file")
    argspars.add_argument("--exclude",nargs='+', default='None', type=str, help="List of serials to not exclude")
    argspars.add_argument("--path_to_project", default=r'.\Data\experiments\new\LIHS\ROM2\calibration', type=str, help="List of serials to not exclude")
    argspars.add_argument("--not_exclude",nargs='+', default='all', type=str, help="List of serials to not exclude")
