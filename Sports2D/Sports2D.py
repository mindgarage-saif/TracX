...
CONFIG_HELP = {
    ...
    "video_input": [
        "i",
        "webcam, or video_path.mp4, or video1_path.avi video2_path.mp4 ... Beware that images won't be saved if paths contain non ASCII characters",
    ],
    "webcam_id": ["w", "webcam ID. 0 if not specified"],
    "time_range": [
        "t",
        "start_time, end_time. In seconds. Whole video if not specified",
    ],
    ...
    "display_angle_values_on": [
        "a",
        '"body", "list", "body" "list", or "None". body list if not specified',
    ],
    "show_graphs": [
        "G",
        "Show plots of raw and processed results. true if not specified",
    ],
    "joint_angles": [
        "j",
        '"Right ankle" "Left ankle" "Right knee" "Left knee" "Right hip" "Left hip" "Right shoulder" "Left shoulder" "Right elbow" "Left elbow" if not specified',
    ],
    "segment_angles": [
        "s",
        '"Right foot" "Left foot" "Right shank" "Left shank" "Right thigh" "Left thigh" "Pelvis" "Trunk" "Shoulders" "Head" "Right arm" "Left arm" "Right forearm" "Left forearm" if not specified',
    ],
    ...
    "pose_model": [
        "p",
        "Only HALPE_26 is available for now. HALPE_26 if not specified",
    ],
    "mode": ["m", "light, balanced, or performance. balanced if not specified"],
    "det_frequency": [
        "f",
        "Run person detection only every N frames, and inbetween track previously detected bounding boxes. keypoint detection is still run on all frames.\n\
    ...
    "multiperson": [
        "M",
        "Multiperson involves tracking: will be faster if set to false. true if not specified",
    ],
    "tracking_mode": [
        "",
        "sports2d or rtmlib. sports2d is generally much more accurate and comparable in speed. sports2d if not specified",
    ],
    "input_size": [
        "",
        "width, height. 1280, 720 if not specified. Lower resolution will be faster but less precise",
    ],
    "keypoint_likelihood_threshold": [
        "",
        "Detected keypoints are not retained if likelihood is below this threshold. 0.3 if not specified",
    ],
    "average_likelihood_threshold": [
        "",
        "Detected persons are not retained if average keypoint likelihood is below this threshold. 0.5 if not specified",
    ],
    "keypoint_number_threshold": [
        "",
        "Detected persons are not retained if number of detected keypoints is below this threshold. 0.3 if not specified, i.e., i.e., 30 percent",
    ],
    "fontSize": ["", "Font size for angle values. 0.3 if not specified"],
    "flip_left_right": [
        "",
        "true or false. true to get consistent angles with people facing both left and right sides. Set it to false if you want timeseries to be continuous even when the participent switches their stance. true if not specified",
    ],
    "interpolate": ["", "Interpolate missing data. true if not specified"],
    "interp_gap_smaller_than": [
        "",
        "Interpolate sequences of missing data if they are less than N frames long. 10 if not specified",
    ],
    "fill_large_gaps_with": [
        "",
        "last_value, nan, or zeros. last_value if not specified",
    ],
    "filter": ["", "Filter results. true if not specified"],
    "filter_type": [
        "",
        "butterworth, gaussian, median, or loess. butterworth if not specified",
    ],
    "order": ["", "Order of the Butterworth filter. 4 if not specified"],
    "cut_off_frequency": [
        "",
        "Cut-off frequency of the Butterworth filter. 3 if not specified",
    ],
    "sigma_kernel": ["", "Sigma of the gaussian filter. 1 if not specified"],
    "nb_values_used": [
        "",
        "Number of values used for the loess filter. 5 if not specified",
    ],
...
def base_params(config_dict):
    ...
    if video_input == "webcam" or video_input == ["webcam"]:
        ...
        for video_file in video_files:
            video = (
                cv2.VideoCapture(str(video_dir / video_file))
                if video_dir
                else cv2.VideoCapture(str(video_file))
            )
            if not video.isOpened():
                raise FileNotFoundError(
                    f"Error: Could not open {video_dir/video_file}. Check that the file exists."
                )
            frame_rate = video.get(cv2.CAP_PROP_FPS)
            if frame_rate == 0:
                frame_rate = 30
                logging.warning(
                    f"Error: Could not retrieve frame rate from {video_dir/video_file}. Defaulting to 30fps."
                )
        ...
        if time_ranges.shape == (0,):
            ...
            raise ValueError(
                "Time range must be [] for analysing all frames of all videos, or [start_time, end_time] for analysing all videos from start_time to end_time, or [[start_time1, end_time1], [start_time2, end_time2], ...] for analysing each video for a different time_range."
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Use sports2d to compute your athlete's pose, joint, and segment angles. See https://github.com/davidpagnon/Sports2D"
    )
    parser.add_argument(
        "-C",
        "--config",
    ...
    for leaf_name in list(CONFIG_HELP.keys())[1:]:
        ...
        arg_str = (
            [f"-{short_key}", f"--{leaf_name}"] if short_key else [f"--{leaf_name}"]
        )
        if type(leaf_keys[leaf_name]) == bool:
            ...
            parser.add_argument(
                *arg_str, type=list_type, nargs="*", help=CONFIG_HELP[leaf_name][1]
            )
        else:
            parser.add_argument(
                *arg_str,
                type=type(leaf_keys[leaf_name]),
                help=CONFIG_HELP[leaf_name][1],
            )
    ...
    if args.config:
        ...
        if not args.video_input:
            new_config.get("project").update(
                {"video_dir": Path(__file__).resolve().parent / "Demo"}
            )
